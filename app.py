...
# Cấu hình thư mục
IMAGE_FOLDER = './data/vg_1000_focused/images'  # Thư mục chứa hình ảnh
...
# Load mô hình RelTR
# model = load_model('./RelTR/ckpt/fine_tune/checkpoint0049.pth')
        ...
        for pred in predictions:
            ...
            for record in result:
                ...
                results[pair_key].append({
                    "image_id": image_id,
                    "url": f"data/vg_1000_focused/images/{image_id}.jpg",
    ...
    try:
        ...
        for category in related_images:
            ...
            for image in related_images[category]:
                image_path = os.path.join(app.config['IMAGE_FOLDER'], f"{image['image_id']}.jpg")
                if os.path.exists(image_path):
                    image["url"] = f"data/vg_1000_focused/images/{image['image_id']}.jpg"
                    category_total += 1
...
@app.route('/data/vg_1000_focused/images/<path:filename>')
def serve_image(filename):
    return send_from_directory(app.config['IMAGE_FOLDER'], filename)

def get_image_url(image_id):
    return f'data/vg_1000_focused/images/{image_id}.jpg'
