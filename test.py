...
def load_model_and_predict(image_path, model_path):
    ...
    try:
        ...
        print(f"Kiểu dữ liệu dự đoán thô: {type(raw_predictions)}")
        print(f"Dữ liệu dự đoán thô: {raw_predictions}")
        ...
        if not isinstance(raw_predictions, list):
            print(f"Cảnh báo: Dự đoán không phải là list: {type(raw_predictions)}")
            # Thử chuyển đổi thành list nếu có thể
            if isinstance(raw_predictions, (int, float, str)):
                print("Chuyển đổi dự đoán đơn lẻ thành list")
        ...
        for pred in raw_predictions:
            if isinstance(pred, dict):
                ...
                # Thử chuyển đổi thành dictionary nếu có thể
                if isinstance(pred, (list, tuple)) and len(pred) >= 3:
                    try:
                        ...
                        print(f"Không thể chuyển đổi dự đoán {pred}: {e}")
                
        print(f"Số lượng dự đoán hợp lệ: {len(valid_predictions)}")
        if len(valid_predictions) > 0:
            print(f"Ví dụ dự đoán hợp lệ: {valid_predictions[0]}")
